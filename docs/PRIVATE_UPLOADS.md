...
After a clean scan, the owning creator may supply WebVTT directly through the protected transcript API. Drafts remain separate from originals and preview-only; only explicit approval moves the source to `ready`. Replacing an approved transcript revokes readiness/public visibility, and deleting the source overwrites caption content and timing metadata in the same database transaction before stored-video reconciliation begins.
