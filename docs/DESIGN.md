...
The current upload slice accepts only `.mp4`/`video/mp4` through a 10-minute exact-size policy and promotes a successful object only to `uploaded`/preview. A one-shot worker uses an exclusive database lease and at most two 512 KB range reads to inspect a supported `ftyp`, complete `moov` metadata, a 1-second-to-4-hour duration, H.264 video, and AAC audio. It then streams the exact object to private ClamAV; only a clean verdict moves to `processing`, with detected duration/codecs and bounded malware status persisted. Infection deletes and disables the object, while scanner/storage failures retry and fail closed. This is not a full decode safety proof. The next sandboxed boundary must add checksum verification and full decoder validation before any automatic transcription adapter receives the object. A source cannot become public until a later reviewed pipeline places it in `ready` state.
